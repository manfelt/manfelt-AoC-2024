...
FILE *f;
char line[SIZE];
int cur, rowlen, *pcur, mul_flag, do_flag;
// TODO char bol, row;
...
int main() {
	...
	while (fgets(line, SIZE, f) != NULL) {
		...
		while (cur<rowlen) {
			cur++;
			if (line[cur]==40) {
				if (cur >= 3 && line[cur-1]==116 && line[cur-2]==39 &&line[cur-3]==110 && line[cur-4]==111 && line[cur-5]==100) {
					do_flag = 0;
					printf("\n 'don't(' found at cur %d", cur);
				}
				if (cur >= 3 && line[cur-1]==111 && line[cur-2]==100) {
					do_flag = 1;
					//if (DEBUG) {
					printf("\n 'do(' found at cur %d", cur);
					//}
				};
